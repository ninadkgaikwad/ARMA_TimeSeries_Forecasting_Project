function [ ProcessedData ,filename] = SolarDataCleaner( DataFile_11,Headers,Res,hem,Ltm,L,N,DateTimeString,NegativeCol,SolarRegionalTime,FileName)
...
%CsvData_Table = readtable(FileName);

DataFile = DataFile_11(:,2:3);
       ...
       elseif Headers==0 % Use Index as 'i'          
           ...
           % Reading Date Time Signature of Current Data Row
            ...
            for k=2:(DataCols+1)
                ...
                DataCapture(1,k-1)=DataFile{i,k};
...
%% Addition of the Last Row for conversion Function

Y=ProcessedData(end,3);
M=ProcessedData(end,2);
D=ProcessedData(end,1);
...
ProcessedData=vertcat(ProcessedData,[D1,M1,Y1,zeros(1,DataCols)]);

% ProcessedData_T=array2table(ProcessedData);
...
filename = [FileName,'_ProcessedFile.xlsx'];

% writetable(ProcessedData_T,filename,'Delimiter',',','QuoteStrings',true);
